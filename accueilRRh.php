...
// Affiche le nombre de notifications dans un badge
echo '<span class="badge badge-primary">' . $donnees['notifications'] . '</span>';*/
...
?>
<!--<a href="congepre.php" id="bell">
    <i class="fa fa-bell"></i>-->
    <?php
    // Affiche le nombre de notifications actuelles de l'administrateur
    /*if(isset($donnees['notifications'])) {
        echo '<span class="badge badge-primary">' . $donnees['notifications'] . '</span>';
    }
    ?></a>
      <?php
// Vérifie si l'administrateur a cliqué sur la cloche
if(isset($_GET['reset_notifications'])) {
    
    // Réinitialise le nombre de notifications à zéro
    $req = $bdd->prepare("UPDATE conges SET notifications = 0 WHERE matricule = ?");
    $req->execute(array($_SESSION['matricule']));
    
    // Redirige l'utilisateur vers la page actuelle pour rafraîchir la page
